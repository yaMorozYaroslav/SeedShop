import styled from 'styled-components'

export const Container = styled.div`
                      position: fixed;
                      top:10%;
                      left:30%;
                      background: yellow;
                      display: block;
		              text-align:center
                       `
export const Title = styled.h1`font-size:30px`
export const Form = styled.form`font-size:24px`
export const Input = styled.input`fontSize:22px;margin:7px;width:50%`
export const Description = styled.textarea`fontSize:22px;margin:7px;height:50px;marginLeft:45px;`
export const Category = styled.select`fontSize:22px;margin:7px;cursor:pointer`
